...
int database_over_ranks(int argc, char **argv, int myRank, int numberProcesses)
{
...
#pragma omp parallel
    ...
    // I don't parallelize this for with OpenMP since it's already very fast in sequential programming
    // Grab the patterns from command line
    ...
    if (myRank == 0)
    {
        ...
        int numberPiecesDatabase = numberProcesses -
...
#if DEBUG
        printf("Rank 0. Size of the database: %d. Number of pieces of database: %d. Size of a piece: %d\n", size_database,
               numberPiecesDatabase, size_piece);
#endif
        ...
        for (j = 1; j < numberProcesses; j++)
        {
            ...
            // If I am the last rank I take all the elements from my startingPoint until the end of the database. If the division of the database had a remainder (left characters) I'll take it.
            ...
            {
                indexStartMyPiece = (j - 1) * size_piece;
                indexFinishMyPieceWithoutExtra = indexStartMyPiece +
        ...
        for (i = 0; i < nb_patterns; i++)
        {
            ...
            for (j = 1; j < numberProcesses; j++)
            {
                ...
                int numberMatches;
                MPI_Status status;
                MPI_Recv(&numberMatches, 1, MPI_INT, MPI_ANY_SOURCE, i, MPI_COMM_WORLD, &status);

#if DEBUG
                printf("Rank 0. I have received number of matches of pattern %d from rank %d: %d\n", i,
                       status.MPI_SOURCE, numberMatches);
#endif
...
#pragma omp parallel default(none) private(i) firstprivate(indexFinishMyPieceWithoutExtra, indexStartMyPiece, n_bytes, approx_factor, nb_patterns, numberProcesses, myRank) shared(buf, pattern, stderr, ompi_mpi_comm_world, ompi_mpi_int, numbersOfMatch)
        {
...
#pragma omp for schedule(static)
            ...
            for (i = 0; i < nb_patterns; i++)
            {
...
#if DEBUG
                printf("----- MPI %d (out of %d) & OpenMP %d (out of %d). Started to analize pattern n° %d.\n", myRank, numberProcesses, omp_get_thread_num(), omp_get_num_threads(), i);
#endif
                ...
                // If I am not the last rank I should take in consideration extra characters from the next piece: in this way I don't miss words which are placed between two pieces.
                // If am the last rank I don't take extra characters as the other ranks since the file is finished.
                int indexFinishMyPieceWithExtra = indexFinishMyPieceWithoutExtra;
...
#if DEBUG
                printf("Rank %d. I received the info from rank 0. Start index: %d. Finish index: %d\n", myRank,
                       indexStartMyPiece, indexFinishMyPieceWithoutExtra);
                printf("Rank %d. Final index updated: %d.\n", myRank, indexFinishMyPieceWithExtra);
#endif

#if DEBUGPIECEREAD
                printf("Rank %d: I will read the following text:\n", myRank);
                int j;
                for (j = indexStartMyPiece; j < indexFinishMyPieceWithExtra - approx_factor; j++)
                {
                    printf("%c", buf[j]);
                }
                printf("\n");
...
#if DEBUG
                printf("----- MPI %d (out of %d) & OpenMP %d (out of %d). Index Start: %d. Index finish: %d.\n", myRank, numberProcesses, omp_get_thread_num(), omp_get_num_threads(), indexStartMyPiece, n_bytes);
#endif

                timestampStart = omp_get_wtime();

                // It's not possible to parallelize with OpenMP this for since the cycles are interconnected.
                int r;
                for (r = indexStartMyPiece; r < n_bytes - approx_factor; r++)
                {

#if DEBUGBYTEOPENMP
                    printf("MPI %d (out of %d) & OpenMP %d (out of %d). I am analyzing byte %d for pattern %d\n", myRank, numberProcesses, omp_get_thread_num(), omp_get_num_threads(), r, i);
                    ...
                    printf("Pattern: %p. Buf: %p. Size: %p. Columns: %p.\ni address: %p. i value: %d. j address: %p. j value: %d \n", &pattern, &buf[j], &size, &column, &i, i, &j, j);
#endif
